...
class Encoder(nn.Module):
    def __init__(self, in_channels=1, channel_num=8):
        ...
        self.conv_2_skip = Conv3DReLUBlock(channel_num, channel_num * 2)
        ...
        self.conv_3_skip = Conv3DReLUBlock(channel_num * 2, channel_num * 4)
        ...
        self.conv_4_skip = Conv3DReLUBlock(channel_num * 4, channel_num * 8)
        ...
        self.conv_5_skip = Conv3DReLUBlock(channel_num * 8, channel_num * 16)
    ...
    def forward(self, x_in):
        ...
        x_2 = self.conv_2(x)
        x_2_skip = self.conv_2_skip(x)
        x_2 = x_2 + x_2_skip
        x = self.downsample(x_2)

        x_3 = self.conv_3(x)
        x_3_skip = self.conv_3_skip(x)
        x_3 = x_3 + x_3_skip
        x = self.downsample(x_3)

        x_4 = self.conv_4(x)
        x_4_skip = self.conv_4_skip(x)
        x_4 = x_4 + x_4_skip
        x = self.downsample(x_4)

        x_5 = self.conv_5(x)
        x_5_skip = self.conv_5_skip(x)
        x_5 = x_5+ x_5_skip
...
class RegMamba(nn.Module):
    def __init__(self, channel_num=16):
        ...
        # 1,4 a; 2,5 c; 3,6 s;
        post = torch.tensor([0.0, 5.0, 0.0, 0.0, 5.0, 0.0])  
        # post = torch.tensor([])  
        ...
        self.conv_fine_1 = Conv3DReLUBlock(channel_num * 1 + channel_num * 1 + 3, channel_num * 1)
        self.conv_fine_2 = Conv3DReLUBlock(channel_num * 2 + channel_num * 2 + 3, channel_num * 2)
        self.conv_fine_3 = Conv3DReLUBlock(channel_num * 4 + channel_num * 4 + 3, channel_num * 4)
        self.conv_fine_4 = Conv3DReLUBlock(channel_num * 8 + channel_num * 8 + 3, channel_num * 8)
